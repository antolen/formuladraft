    ...
    <main className="min-h-screen p-8 sm:p-24 sm:py-12">
      ...
      <ul>
        {peopleKeys.map((team, index) => (
          <li className="flex bg-white my-1 py-5 px-4 rounded-md" key={team}>
      ...
      <table className="w-full text-left whitespace-nowrap table-fixed p-2">
        <thead className="bg-black text-white w-full">
          <tr className="w-full mb-4">
            <th className="bg-black p-2 p-2 sticky left-0 z-10 w-[180px]">Driver</th>
            {allRaces.MRData.RaceTable.Races.map((race) => (
              <th key={race.raceName} className="p-2 whitespace-nowrap sticky  top-0 w-[130px]">
        ...
        <tbody>
          {Object.keys(driverStats).map((driver) => (
            <tr key={driver} className="bg-white rounded m-2">
